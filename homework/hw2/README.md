...
Класс, который требует рефакторинга: ConsumerService

##4 Применение SOLID  
В классе ConsumerService нарушены принципы SOLID: разделите ответственности (SRP), выделите абстракции (DIP/ISP).

Что было сделано: выделены две Entity: TicketEntity и ServiceObjectEntity. В TicketEntity использовал Value Objeсt Email. В TicketEntity и ServiceObjectEntity реализован метод save, который позволяет сохранить Entity в БД.
